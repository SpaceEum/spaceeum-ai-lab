...
def execute_trade(symbol, signal, atr):
    try:
        ...
        if signal == 'LONG':
            ...
            # Place SL/TP
            # Binance Futures requires specific params for Algo orders
            try:
                sl_params = {'stopPrice': sl_price, 'reduceOnly': True}
                exchange.create_order(symbol, 'STOP_MARKET', 'sell', amount, params=sl_params)
            ...
            try:
                ...
                exchange.create_order(symbol, 'TAKE_PROFIT_MARKET', 'sell', amount, params=tp_params)
            ...
            try:
                sl_params = {'stopPrice': sl_price, 'reduceOnly': True}
                exchange.create_order(symbol, 'STOP_MARKET', 'buy', amount, params=sl_params)
                logger.info(f"SL Placed for {symbol} at {sl_price}")
                log_trade_to_db({
                    'timestamp': datetime.now().strftime('%Y-%m-%d %H:%M:%S'),
                    'symbol': symbol,
            ...
            try:
                tp_params = {'stopPrice': tp_price, 'reduceOnly': True}
                exchange.create_order(symbol, 'TAKE_PROFIT_MARKET', 'buy', amount, params=tp_params)
                logger.info(f"TP Placed for {symbol} at {tp_price}")
                log_trade_to_db({
                    'timestamp': datetime.now().strftime('%Y-%m-%d %H:%M:%S'),
                    'symbol': symbol,
