# test_tenkan_quick.py
# 'tenkan_backtest_v1.py'의 테스트 버전 (Top 3 티커만 실행)
...
def load_and_filter_tickers(timeframe='15m', top_n=3):
    """
    ...
    print(f"[{timeframe}] 티커 필터링 시작 (테스트용, 전체 스캔 중)...")
    
    # 테스트를 위해 앞의 20개 파일만 스캔해서 그 중 상위 3개를 뽑음 (속도 최적화)
    scanned_count = 0
    for filename in all_files:
        if scanned_count >= 20: break 
    ...
    print(f"필터링 완료: {len(top_tickers)}개 티커 선정 (테스트용).")
    return top_tickers
...
def run_tenkan_strategy(df, fee_rate, leverage, trailing_stop_pct=None, stop_loss_pct=None, take_profit_pct=None):
    ...
    for i in range(1, len(df)):
        ...
        if not in_position:
            ...
            # 알반갭 확인 (10기간 고가 갱신) -> 현재 캔들이 10기간 고가를 만듦 (즉, period_high_10 상승)
            alban_gap = curr['period_high_10'] > prev['period_high_10']
            ...
            if is_negative_cloud:
                span_a_rising = curr['span_a'] > prev['span_a']
                if not span_a_rising:
                    continue # 음운인데 할머니(Span A)가 고개를 안 들면 매수 금지

            # 진입 신호 판별
            if high_9_rising and low_9_rising:
                detected_type = '200%_Rise' # 고가/저가 동시 상승 (매우 강력)
            elif alban_gap:
                detected_type = 'Alban_Gap' # 알반갭 (강력)
            elif high_9_rising and not low_9_rising:
                detected_type = '100%_Rise' # 고가만 상승 (일반)
...
# --- 메인 실행 ---
def run_v1_backtest():
    print("=== 전환선 전략 v1 테스트 (Quick) 시작 ===")
    
    tickers = load_and_filter_tickers('15m', top_n=3) 
    if not tickers:
        print("티커를 찾을 수 없습니다.")
        return
    ...
    for idx, ticker in enumerate(tickers):
        ...
        try:
            ...
            print(f"{ticker}: {len(trades)} trades generated.")
    ...
    filename = f"tenkan_test_result_{datetime.now().strftime('%Y%m%d_%H%M%S')}.db"
