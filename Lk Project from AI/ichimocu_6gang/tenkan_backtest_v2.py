# tenkan_backtest_v2.py
# 'stratege.md'의 II. 전환선 매매 전략 + IV. 선행스팬 1 매매 전략 (v2)
...
def load_and_filter_tickers(timeframe='15m', top_n=200):
    ...
    print(f"[{timeframe}] 티커 필터링 시작 (기준상태: 최근 1년, {len(all_files)}개 파일)...")
...
# --- 핵심 전략 로직 ---
def run_v2_strategy(df, fee_rate, leverage):
    """
    V1(전환선) + V2(선행스팬 1 지지/저항) 매매 시뮬레이션
    """
    ...
    for i in range(1, len(df)):
        ...
        if entry_stage == 0:
            if v1_signal and v2_low_breakout:
                ...
                entry_log.append(f"{v1_signal}&LowBreak(Wait)")

        # Case B: 관망 중 결정 (진입 or 취소)
        elif entry_stage == 1:
            # 1. 진입 조건: 고점 돌파 시 100% 진입
            if v2_high_breakout:
                # 진입 수량 및 자금 계산
...
def run_v2_backtest():
    ...
    print("=== 전환선(V1) + 선행스팬1(V2) 전략 백테스트 시작 ===")
    ...
    for idx, ticker in enumerate(tickers):
        ...
        try:
            ...
            # 메인 실행에서도 1년 데이터 필터링 적용
            df_recent = df[df['timestamp'] >= one_year_ago].copy()
            if df_recent.empty: continue
            
            trades = run_v2_strategy(df_recent, FEE_RATE, LEVERAGE)
    ...
    filename = f"tenkan_v2_result_{datetime.now().strftime('%Y%m%d_%H%M%S')}.db"
