...
# --- 설정 ---
# 데이터 최상위 폴더
OHLCV_DIR = r"G:\내 드라이브\LK Project\Lk Project from AI\OHLCV"
...
def run_strategy_md_backtest(df, config):
    ...
    # --- ✨ 수정: 진입/청산/역배열 조건에 필요한 모든 지표 계산 ---
    sma_series = ta.sma(df['close'], length=config['ma_period'])
    if sma_series is None:
        logging.warning(f"[{config.get('ticker', '')}] SMA_60 지표 계산 실패 (데이터 부족 가능성). 건너뜁니다.")
        return None
    df['SMA_60'] = sma_series
    ...
    if isinstance(ichimoku_df, tuple):
        # 계산 실패 시 (None, None) 튜플이 반환될 수 있음
        if ichimoku_df[0] is None or ichimoku_df[1] is None:
            logging.warning(f"[{config.get('ticker', '')}] 일목균형표 지표 계산 실패 (데이터 부족 가능성). 건너뜁니다.")
            return None
        ichimoku_df = pd.concat([ichimoku_df[0], ichimoku_df[1]], axis=1)
    ...
    # 컬럼 이름 표준화 (진입/청산/역배열에 필요한 모든 컬럼)
    # ITS_9, IKS_26, ISA_9_26_52, ISB_26_52
    expected_cols = ['ITS_9', 'IKS_26', 'ISA_9_26_52', 'ISB_26_52']
    ichimoku_subset = ichimoku_df.iloc[:, [0, 1, 3, 4]]
    ichimoku_subset.columns = expected_cols
    df = df.join(ichimoku_subset)

    df['close_shifted_60'] = df['close'].shift(config['ma_period'])
    df['sma_shifted_26'] = df['SMA_60'].shift(config['ichi_kijun'])
    df['close_shifted_26'] = df['close'].shift(config['ichi_kijun'])

    # --- ✨ 추가: 역배열 조건 및 5기간 지속 여부 계산 ---
    df['reverse_condition'] = (
        (df['close'] < df['ISA_9_26_52']) &
        (df['ISA_9_26_52'] < df['ISB_26_52']) &
        (df['close'] < df['SMA_60']) &
    ...
    df['reverse_sustained'] = df['reverse_condition'].rolling(window=5).apply(lambda x: x.all(), raw=True).fillna(0).astype(bool)
    ...
    # --- ✨ 요청사항: 역배열 신호를 기억하는 상태 변수(래치) 추가 ---
    reverse_signal_latched = False
    ...
    for i in range(1, len(df)):
        ...
        if in_position:
            ...
            if not exit_triggered and prev_row['close'] >= prev_row['ISB_26_52'] and current_row['close'] < current_row['ISB_26_52']:
                exit_triggered = True
            ...
            if exit_triggered:
                ...
                # --- ✨ 요청사항: 거래 종료 시, 역배열 신호 래치 리셋 ---
                reverse_signal_latched = False

        # --- 2. 포지션 미보유 시, 진입 조건 확인 ---
        if not in_position:
            # --- ✨ 요청사항: 역배열 신호 래칭 및 진입 로직 수정 ---
            
            # 1. 역배열 신호가 아직 켜지지 않았다면, 신호가 발생하는지 확인하여 켭니다.
            if not reverse_signal_latched:
                if current_row['reverse_sustained']:
                    reverse_signal_latched = True
                    logging.debug(f"[{current_row.name}] {config.get('ticker', '')}: 역배열 신호 래치 ON")
            
            # 2. 역배열 신호가 켜져 있다면, 돌파 조건을 확인하여 진입합니다.
            if reverse_signal_latched:
                breakout_conditions = [
                ...
                if all(breakout_conditions):
                    ...
                    logging.debug(f"[{current_row.name}] {config.get('ticker', '')}: 진입 조건 충족. 포지션 진입.")
...
def run_full_backtest_for_timeframe(timeframe):
    ...
    result_db_file = f"result_{timeframe}_02_{datetime.now().strftime('%Y%m%d_%H%M%S')}.db"
    ...
    for i, ticker in enumerate(final_tickers_list):
        ...
        for sl_pct in stop_loss_range:
            ...
            temp_config['ticker'] = ticker # 로깅을 위해 티커 이름 전달
